...
export const runtime = "nodejs"; // important (envoi email + calls externes)
...
function missingFieldError(field: keyof Payload) {
...
export async function POST(req: Request) {
  try {
    ...
    const payload: Payload = {
      ...
      target_company_size: safeStr(raw.target_company_size, 200),
    ...
    // 5) Validation minimale (tous requis sauf submitted_at/email déjà gérés)
    ...
    for (const f of requiredFields) {
      if (!payload[f]) return missingFieldError(f);
    }
